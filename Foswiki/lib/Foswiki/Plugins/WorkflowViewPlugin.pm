...
package Foswiki::Plugins::WorkflowViewPlugin;
...
our $SHORTDESCRIPTION = 'Used to allow users or not to view topics under workflowplugin flow';
...
sub initPlugin {
    ...
    return 0
        unless Foswiki::Func::checkAccessPermission('VIEW', $user, undef, $topic, $web, undef);
    if (Foswiki::Func::getPreferencesFlag("WORKFLOW")) {
        if (!Foswiki::Func::isGroupMember("WorkflowChangeGroup", undef) && !Foswiki::Func::isAnAdmin()) {
	    my ($meta, $text) = Foswiki::Func::readTopic($web, $topic);
            my $wfmeta = $meta->get('WORKFLOW');
	    my $arev = $wfmeta->{'LASTVERSION_APROBADO'};
	    Foswiki::Func::writeWarning($arev);
	    my $qry = Foswiki::Func::getCgiQuery();
	    if($arev) {
	        $qry->param(-name => 'rev', -value => $arev);
	    }
	    elsif(!Foswiki::Func::getContext()->{'oops'}) {
	        $qry->param(-name => 'template', -value => 'System.NoApprovedVersionError');
	        Foswiki::Func::redirectCgiQuery(
		    undef, Foswiki::Func::getScriptUrl($web,$topic,'oops'), 1);
	    }
        }
    }
    ...
    # like this:
    # $Foswiki::cfg{Plugins}{WorkflowViewPlugin}{ExampleSetting} = 1;
    ...
    # my $setting = $Foswiki::cfg{Plugins}{WorkflowViewPlugin}{ExampleSetting} || 0;
...
This handler can be used to replace Foswikis internal redirect function.
...
1;
__END__
This copyright information applies to the WorkflowViewPlugin:

# Plugin for Foswiki - The Free and Open Source Wiki, http://foswiki.org/
#
# WorkflowViewPlugin is Copyright (C) 2008 Foswiki Contributors. Foswiki Contributors
# are listed in the AUTHORS file in the root of this distribution.
# NOTE: Please extend that file, not this notice.
# Additional copyrights apply to some or all of the code as follows:
# Copyright (C) 2000-2003 Andrea Sterbini, a.sterbini@flashnet.it
# Copyright (C) 2001-2006 Peter Thoeny, peter@thoeny.org
# Copyright (C) 2010 Hexa Ingenieros Asociados S.A.S., soluciones@hexasas.com
# and TWiki Contributors. All Rights Reserved. Foswiki Contributors
# are listed in the AUTHORS file in the root of this distribution.
#
# This license applies to WorkflowViewPlugin *and also to any derivatives*
#
